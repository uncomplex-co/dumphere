...
class McpOAuthService(
    ...
    fun validateAuthorizationRequest(
        ...
        val resolvedRedirectUri = redirectUri?.takeIf { it.isNotBlank() } ?: throw OAuthRequestException("invalid_request", "Missing redirect_uri")
        ...
        val resolvedState = state?.takeIf { it.isNotBlank() } ?: throw OAuthRequestException("invalid_request", "Missing state", resolvedRedirectUri)
        val resolvedCodeChallenge = codeChallenge?.takeIf { it.isNotBlank() }
        ...
        val requestedScopes = scope.orEmpty()
            .split(' ')
    ...
    fun issueAuthorizationCode(request: McpAuthorizationRequest, user: AuthenticatedUser): String {
        pruneExpiredGrants()
        val code = randomToken(32)
        grants[code] = AuthorizationCodeGrant(
            request = request,
            user = user,
    ...
    fun exchangeAuthorizationCode(
        ...
        val resolvedRedirectUri = redirectUri?.takeIf { it.isNotBlank() } ?: throw OAuthTokenException("invalid_request", "Missing redirect_uri")
    ...
    fun authorizationSuccessRedirect(request: McpAuthorizationRequest, code: String): String = buildString {
    ...
    fun authorizationErrorRedirect(request: McpAuthorizationRequest, error: String): String = buildString {
        append(request.redirectUri)
        append(if ('?' in request.redirectUri) '&' else '?')
        append("error=")
    ...
    private fun authenticateClient(
        ...
        val basic = authorizationHeader?.takeIf { it.startsWith("Basic ", ignoreCase = true) }
            ?.removePrefix("Basic ")
            ?.let { encoded ->
                val decoded = String(Base64.getDecoder().decode(encoded), UTF_8)
                val separator = decoded.indexOf(':')
                if (separator < 0) throw OAuthTokenException("invalid_client", "Malformed basic authentication", HttpStatus.UNAUTHORIZED)
                decoded.substring(0, separator) to decoded.substring(separator + 1)
            }

        val clientId = basic?.first ?: clientIdParam?.takeIf { it.isNotBlank() }
            ?: throw OAuthTokenException("invalid_client", "Missing client authentication", HttpStatus.UNAUTHORIZED)
        val clientSecret = basic?.second ?: clientSecretParam?.takeIf { it.isNotBlank() }
    ...
    private fun String.sha256UrlSafe(): String = Base64.getUrlEncoder().withoutPadding().encodeToString(
