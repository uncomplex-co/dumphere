...
class SecurityConfig(
    ...
    fun securityFilterChain(http: HttpSecurity): SecurityFilterChain {
        http
            .csrf { csrf -> csrf.ignoringRequestMatchers(matcher("/mcp"), matcher("/api/**"), matcher("/connect/register"), matcher("/oauth2/token")) }
            .requestCache { cache -> cache.requestCache(requestCache()) }
            .authorizeHttpRequests { requests ->
                requests
                    .requestMatchers("/error", "/login/**", "/oauth2/authorization/**", "/login/oauth2/**").permitAll()
                    .requestMatchers(
                    ...
                    .anyRequest().access { authentication, _ ->
                        ...
                        AuthorizationDecision(auth.isAuthenticated && auth !is AnonymousAuthenticationToken && emailAllowed(auth))
                    }
            }
            .oauth2Login { oauth ->
    ...
    private fun loginEntryPoint() = org.springframework.security.web.authentication.LoginUrlAuthenticationEntryPoint("/login")
    ...
    @Bean
    fun authTimeAuthoritiesMapper(): GrantedAuthoritiesMapper = GrantedAuthoritiesMapper { authorities ->
    ...
    private fun Collection<GrantedAuthority>.withAuthenticationTime(): Collection<GrantedAuthority> =
        if (any { it is FactorGrantedAuthority }) {
            ...
            this + FactorGrantedAuthority.withAuthority(FactorGrantedAuthority.AUTHORIZATION_CODE_AUTHORITY)
    ...
    private fun emailAllowed(authentication: org.springframework.security.core.Authentication): Boolean {
        ...
        val email = when (val principal = authentication.principal) {
            ...
            is org.springframework.security.oauth2.core.OAuth2AuthenticatedPrincipal -> principal.getAttribute("email")
            else -> null
        } ?: return false
